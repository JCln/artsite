...
export class AuthGuard implements CanActivate, CanActivateChild, CanLoad {
  ...
  canActivate(
    ...
    const permissionData = route.data[
      this.permissionObjectKey
  ...
  canActivateChild(
    ...
    const permissionData = childRoute.data[
      this.permissionObjectKey
    ] as AuthGuardPermission;
    const returnUrl = state.url;
    return this.hasAuthUserAccessToThisRoute(permissionData, returnUrl);
  ...
  private hasAuthUserAccessToThisRoute(
    permissionData: Data,
    returnUrl: string
  ): boolean {
    ...
    if (Array.isArray(permissionData.permittedRoles)) {
      const isInRole = this.authService.isAuthUserInRoles(
